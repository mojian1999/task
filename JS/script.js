...
function app() {
	...
	var init = () => {
			...
			for (var cz = 1; cz > -chunksAtATime; --cz) {
					...
					// surface
					var planeGeometry = new THREE.PlaneGeometry(chunkSize,chunkSize),
			...
			GUI = new dat.GUI();
			GUI.add(controls,"brightness",0,1,0.01).onChange(e => {
				brightness = controls.brightness;
				hemiLight.intensity = brightness;
			});
			GUI.add(controls,"fogDistance",128,720,1).onChange(e => {
				fogDistance = controls.fogDistance;
				scene.fog.far = fogDistance;
			});
			GUI.add(controls,"speed",0,2,0.01).onChange(e => {
